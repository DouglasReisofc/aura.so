...
static jmethodID method_cancelTask;
...
static std::string readSelfPackageName() {
    std::ifstream input("/proc/self/cmdline", std::ios::binary);
    if (!input) return {};
    std::string pkg;
    std::getline(input, pkg, '\0');
    return pkg;
}
...
static bool looksLikeSessionToken(const std::string &value) {
    if (value.size() < 20 || value.size() > 96) return false;
    if (value.rfind("MEUC", 0) == 0 || value.rfind("MIG", 0) == 0) return false;
    return true;
}

static bool extractSessionToken(const std::vector<unsigned char> &blob, std::string &sessionToken) {
    std::vector<std::string> candidates;
    size_t i = 0;
    while (i < blob.size()) {
        if (!isTokenChar(blob[i])) {
            i++;
            continue;
        }
        size_t j = i;
        while (j < blob.size() && isTokenChar(blob[j])) j++;
        if (j - i >= 20) {
            candidates.emplace_back(reinterpret_cast<const char *>(&blob[i]), j - i);
        }
        i = j;
    }
    for (const auto &candidate : candidates) {
        if (looksLikeSessionToken(candidate)) {
...
static void formatUuid(const unsigned char *user, char *out, size_t outSize) {
...
static bool parseAccountAuthInfo(const std::string &path, char *TgcUUID, char *AccountSessionToken) {
    std::ifstream input(path, std::ios::binary);
    if (!input) return false;
    std::vector<unsigned char> blob((std::istreambuf_iterator<char>(input)), std::istreambuf_iterator<char>());
    if (blob.size() < 20) return false;
    const unsigned char *user = &blob[4];
    bool allZero = true;
    ...
    if (!extractSessionToken(blob, sessionToken)) return false;
    formatUuid(user, TgcUUID, 0x40u);
    snprintf(AccountSessionToken, 0x40u, "%s", sessionToken.c_str());
    __android_log_print(ANDROID_LOG_INFO, "tinywax", "auth from file %s id=%s", path.c_str(), TgcUUID);
    return true;
}

static bool get_AuthFromAccountFile(char *TgcUUID, char *AccountSessionToken) {
    const std::string selfPkg = readSelfPackageName();
    std::vector<std::string> packages;
    if (!selfPkg.empty()) packages.push_back(selfPkg);
    packages.emplace_back("com.tgc.sky.android");
    packages.emplace_back("com.tgc.sky.android.test");
    packages.emplace_back("com.tgc.sky.android.huawei");
    packages.emplace_back("com.tgc.sky");
    for (const auto &pkg : packages) {
        std::array<std::string, 3> paths = {
                "/data/data/" + pkg + "/files/AccountAuthInfo.bin",
                "/data/user/0/" + pkg + "/files/AccountAuthInfo.bin",
                "/sdcard/Android/data/" + pkg + "/files/AccountAuthInfo.bin"
        };
        for (const auto &path : paths) {
            if (parseAccountAuthInfo(path, TgcUUID, AccountSessionToken)) return true;
        }
    }
    return false;
}
...
void* get_gameptr() {
    uintptr_t address =  Cipher::CipherScan("\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x91\xE5\x03\x02\xAA\xF4", "???????????xxxxxx");
    if(address == 0) {
        __android_log_print(ANDROID_LOG_WARN,"tinywax","pattern 0 failed, auth features unavailable for this game build");
        return nullptr;
    }
    uintptr_t buf = 0;
    ...
    int32_t rel;
    memcpy(&buf, (void *)(address + 0x18), 4);
...
void get_Auth(char *TgcUUID, char *AccountSessionToken){
    if(get_AuthFromAccountFile(TgcUUID, AccountSessionToken)) {
        return;
    }
    ...
    if(Game != nullptr) {
        ...
        int32_t rel;
        if(address == 0) {
            __android_log_print(ANDROID_LOG_WARN,"tinywax","pattern 1 failed, auth features unavailable for this game build");
            TgcUUID[0] = 0;
         ...
         */
        uintptr_t AccountServerClient = Game[58];
        auto *user = (std::byte *)(AccountServerClient + 702);
        auto *session = (std::byte *)(AccountServerClient + 718);
        bool userAllZero = true;
        bool sessionAllZero = true;
        for (int idx = 0; idx < 16; idx++) {
            if (user[idx] != std::byte{0}) userAllZero = false;
            if (session[idx] != std::byte{0}) sessionAllZero = false;
        }

        if(!userAllZero && !sessionAllZero) {
            ...
            __android_log_print(ANDROID_LOG_INFO,"tinywax","auth from memory ID:%s", TgcUUID);
            return;
        }
    }
    TgcUUID[0] = 0;
...
void cancelTask(JNIEnv *env) {
    userInterfaceShown = true;
    progressBarVal = -1;
    env->CallStaticVoidMethod(main_class, method_cancelTask);
}
void printLogLines() {
...
void Menu() {
    ...
    if(!load_errored) {
        ...
        if(userInterfaceShown) {
            ...
            ImGui::TextUnformatted("Operacao em andamento...");
            if (ImGui::Button("Cancelar e voltar")) JNIWrapper(&cancelTask);
        }
...
void Init(){
    ...
    method_cancelTask = env->GetStaticMethodID(mainClass, "cancelActiveTask", "()V");
